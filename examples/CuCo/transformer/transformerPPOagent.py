...
class TransformerPPOAC(nn.Module):
    ...
    def forward(self, state, act=None, return_attention=False):
        ...
        num_voxels = self.sequence_size - torch.sum(obs_padding.int(), dim=1, keepdim=True)
        val = torch.divide(torch.sum(module_vals, dim=1, keepdim=True), num_voxels)
...
class Agent(nn.Module):
    ...
    def __init__(self, actor_critic, nca):
    ...
    def forward(self, obs, act):
        ...
        design_idx = np.argwhere(index.cpu().numpy()<1)

        val = torch.zeros(batch_size,1).to(self.nca.device)
        logp = torch.zeros(batch_size,1).to(self.nca.device)
        ent = torch.zeros(batch_size,1).to(self.nca.device)
        ...
        ### design batch
        if design_idx.shape[0]>0:
            design_obs_batch = obs['design'][design_idx[:,0]]
            design_act_batch = act[design_idx[:,0]]
            design_val, design_logp, design_ent = self.nca.design_act(state=design_obs_batch, act=design_act_batch, value_net=self.ac.v_net,mean_action=False,return_attention=False)
            design_logp = torch.squeeze(design_logp,-1)

            val[design_idx[:,0]] = design_val
            logp[design_idx[:,0]] = design_logp
            ent[design_idx[:,0]] = design_ent
    ...
    def uni_act(self, obs, mean_action=False):
        ...
        val = torch.zeros(batch_size,1).to(self.nca.device)
        logp = torch.zeros(batch_size,1).to(self.nca.device)
        act = torch.zeros(batch_size,self.ac.sequence_size).to(self.nca.device)
        ...
        if design_idx.shape[0]>0:
            design_obs_batch = obs['design'][design_idx[:,0]]
            design_act, design_logp, design_val = self.nca.design_act(state=design_obs_batch, value_net=self.ac.v_net,mean_action=mean_action,return_attention=False)
            design_logp = torch.squeeze(design_logp, -1)
            val[design_idx[:,0]] = design_val
            logp[design_idx[:,0]] = design_logp

            for i in range(design_idx.shape[0]):
                act[design_idx[:,0][i]] = design_act[i]
