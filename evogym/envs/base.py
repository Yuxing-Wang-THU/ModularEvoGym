...
class EvoGymBase(gym.Env):
    ...
    # Added by Yuxing
    def get_relative_pos_obs_nof(self, object_name: str):
        """
        Observation helper-function. Computes the positions of a target object's point masses relative to their center of mass.
...
class BenchmarkBase(EvoGymBase):
    ...
    def get_relative_pos_obs_nof(self, object_name):
        return super().get_relative_pos_obs_nof(object_name)*self.VOXEL_SIZE
    ...
    # Set attributes
    def set_modular_attributes(self, body=None, nca_setting=None, init_nca_design=None):
        self.stage = 'act'
    ...
    def get_other_obs(self):
        ...
        if self.env_id in ['Walker-v0', 'Climber-v0','Climber-v1']:
            other_obs = self.get_vel_com_obs("robot")
        elif self.env_id in ['BridgeWalker-v0']: 
            other_obs =  np.concatenate((self.get_vel_com_obs("robot"), self.get_ort_obs("robot"))) 
            ...
            other_obs = np.array([self.object_orientation_at_time(self.get_time(), "robot")]) 
        elif "mizer" in self.env_id:
            other_obs = np.array([0.0, 0.0])
            ...
            # observation
            other_obs = self.get_obs_mani(robot_pos_final, robot_vel_final, package_pos_final, package_vel_final)
            if self.env_id == 'Lifter-v0':
                other_obs = np.concatenate((other_obs,self.get_ort_obs("package")))
            ...
            raise ValueError("Environment not support")  
        return other_obs
    
    def get_modular_obs(self):
        obs = {}
        origin_ob=self.get_relative_pos_obs_nof("robot")
    ...
    def design_step(self, action):
        ...
        if self.cur_t == self.design_steps:
            ...
            act_ob = self.update(body=self.real_design, connections=get_full_connectivity(self.real_design))
